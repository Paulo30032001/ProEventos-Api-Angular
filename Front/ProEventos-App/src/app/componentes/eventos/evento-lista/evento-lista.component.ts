...
export class EventoListaComponent implements OnInit {
  ...
  constructor(
    ...
    private rota : Router //objeto para manipular as rotas
  ) {}
  ...
  public set filtroDeLista(filtrado: string) {
    ...
    this.eventosFiltrado =
      this.filtroLista != null
        ? this.filtrar_lista(this.filtroLista)
        : this.eventos;
  ...
  public GetEventos(): void {
    ...
    const Observer = {
      // funções anonimas () =>{}
      next: (Eventos: Evento[]) => {
        // representa o corpo do response
  ...
  public filtrar_lista(conteudo: string) {
    return this.eventos.filter(
      (evento) =>
        evento.tema.toLowerCase().includes(conteudo.toLowerCase()) ||
        evento.local.toLowerCase().includes(conteudo.toLowerCase())
    );
  }
